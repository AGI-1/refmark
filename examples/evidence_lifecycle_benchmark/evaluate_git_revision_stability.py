...
def main() -> None:
    ...
    parser.add_argument("--repo-url", default="https://github.com/tiangolo/fastapi.git")
    parser.add_argument("--old-ref", default="0.100.0")
    parser.add_argument("--new-ref", default="0.110.0")
    parser.add_argument("--new-refs", default="")
    parser.add_argument("--subdir", default="docs/en/docs")
    parser.add_argument("--work-dir", default="examples/external_qa_benchmark/output/git_revision_work/fastapi")
    parser.add_argument("--output", default="examples/external_qa_benchmark/output/git_revision_fastapi_0_100_0_0_110_0.json")
    ...
    repo = ensure_repo(args.repo_url, work / "repo")
    old_root = export_ref(repo, args.old_ref, args.subdir, work / "old")
    old_regions = load_regions(old_root, region_tokens=args.region_tokens, region_stride=args.region_stride, max_files=args.max_files)
    refs_to_run = [ref.strip() for ref in (args.new_refs or args.new_ref).split(",") if ref.strip()]
    revision_reports = []
    for new_ref in refs_to_run:
        new_root = export_ref(repo, new_ref, args.subdir, work / f"new_{safe_name(new_ref)}")
        new_regions = load_regions(new_root, region_tokens=args.region_tokens, region_stride=args.region_stride, max_files=args.max_files)
    ...
    payload = {
        ...
        "new_ref": refs_to_run[-1] if refs_to_run else args.new_ref,
        ...
        "summary_rows": revision_summary_rows(args.repo_url, args.old_ref, args.subdir, revision_reports),
